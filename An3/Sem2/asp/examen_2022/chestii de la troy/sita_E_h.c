 ...
 * 
 * Utilizare: mpirun -np N ./sita_E_h <n> <nThreads>
 */
...
int processCommandLine(int argc, char ** argv, int * n, int * nthreads)
{
    if (argc == 3) 
    {
      *n = atoi( argv[1] );
...
int main (int argc, char ** argv)
{
   ...
   int my_rank, nproc;
   int numThreads = -1, threadSupportLevel = -1, retval;
   FILE * fp;

   retval = MPI_Init_thread(&argc, &argv, MPI_THREAD_FUNNELED, &threadSupportLevel);
   if( retval != MPI_SUCCESS)
   {
       fp = fopen("error.log","a");
       fprintf(fp, "\nMPI + OpenMP threading nu este suportat,\n");
       fprintf(stderr, " nivelul de suport al bibliotecii OpenMPI = %d\n\n", threadSupportLevel);
       fclose(fp); fp = NULL;
       exit(1);
   }
   ...
   if (processCommandLine(argc,argv,&n,&numThreads)) 
   {
      if (!my_rank) 
      {
         fp = fopen("error.log","a");
         fprintf (fp,"Programul nu a fost lansat corect in executie. \nUtilizare: %s <limita_superioara> <numThreads>\n", argv[0]);
         fclose(fp); fp = NULL;
      }
      MPI_Finalize(); exit(1);
   }
...
#pragma omp parallel for
   ...
   if (!my_rank) 
   {
      ...
      fprintf (fp, "Timpul total de executie cu %d procese si %d fire de executie: %10.6f sec.\n", nproc, numThreads, delta);
      fclose(fp); fp = NULL;
   }
