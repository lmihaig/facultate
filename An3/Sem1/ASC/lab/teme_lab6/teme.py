...
def getUserInput(
    var_name: str, var_type: callable, inf=-math.inf, sup=math.inf
) -> float:
    while True:
...
def ex1():
    ...
    return sum(
        [((-1) ** k * x ** (2 * k)) / (2 * math.factorial(k)) for k in range(n + 1)]
    )
...
def ex6():
    ...
    while abs(math.e - e) >= 1e-6:
        ...
        e = 3 + e_sum
    print(
        "Numarul de iteratii necesare pentru criteriul de convergenta:", k - 1
    )  # indexarea incepe la 2
