...
def deseneazaEcranJoc():
    ...
    for nod in coordonateNoduri:
        pygame.draw.circle(
            surface=ecran, color=culoareLinii, center=nod, radius=Graph.razaPct, width=0
    ...
    for muchie in Graph.muchii:
        ...
        pygame.draw.line(
            surface=ecran, color=culoareLinii, start_pos=p0, end_pos=p1, width=5
        )
    ...
    if nodPiesaSelectata:
        ecran.blit(
            piesaSelectata,
            (
                nodPiesaSelectata[0] - Graph.razaPiesa,
                nodPiesaSelectata[1] - Graph.razaPiesa,
            ),
        )
