...
class Celula:
    ...
    def __init__(self, left, top, w, h, display, lin, col, interfata, cod=0):
        ...
        self.cod = 0
        if lin > 0:
            self.zid[0] = pygame.Rect(
                left,
                top - 1 - self.__class__.grosimeZid // 2,
                w,
                self.__class__.grosimeZid,
            )
        else:
            self.cod += 2**0
        if col < interfata.nrColoane - 1:
            self.zid[1] = pygame.Rect(
                left + w - self.__class__.grosimeZid // 2,
                top,
                self.__class__.grosimeZid,
                h,
            )
        ...
        if lin < interfata.nrLinii - 1:
            self.zid[2] = pygame.Rect(
                left,
                top + h - self.__class__.grosimeZid // 2,
                w,
                self.__class__.grosimeZid,
            )
        else:
            self.cod += 2**2
        if col > 0:
            self.zid[3] = pygame.Rect(
                left - 1 - self.__class__.grosimeZid // 2,
                top,
                self.__class__.grosimeZid,
                h,
            )
    ...
    def deseneaza(self):
        ...
        for i in range(4):
            if self.cod & masca:
                if self.zid[i]:
                    pygame.draw.rect(
                        self.display, self.__class__.culoareLinii, self.zid[i]
                    )
            masca *= 2
...
class Interfata:
    ...
    def __init__(self, nrLinii=7, nrColoane=10, ecran=None):
        ...
        self.matrCelule = [
            [
                Celula(
                    display=ecran,
                    left=col * (self.__class__.dimCelula + 1),
                    top=lin * (self.__class__.dimCelula + 1),
                    w=self.__class__.dimCelula,
                    h=self.__class__.dimCelula,
                    lin=lin,
                    col=col,
                    interfata=self,
                )
                for col in range(nrColoane)
            ]
            for lin in range(nrLinii)
        ]

        furios = pygame.image.load("foarte_furioasa_m.png")

        self.furios = pygame.transform.scale(
            furios, (self.__class__.dimImagine, self.__class__.dimImagine)
        )
        vesel = pygame.image.load("smiley_galben_vesel.png")

        self.vesel = pygame.transform.scale(
            vesel, (self.__class__.dimImagine, self.__class__.dimImagine)
        )
    ...
    def deseneazaImag(self, imag, cel):
        self.ecran.blit(
            imag,
            (
                cel.dreptunghi.left + self.__class__.paddingCelula,
                cel.dreptunghi.top + self.__class__.paddingCelula,
            ),
        )
