...
func FindPerfectSquareCount(words []string) (uint32, error) {
	var count uint32 = 0
	for _, s := range words {
...
func SumOfReversedIntegers(numbers []int32) (int32, error) {
	var sum int32 = 0
...
func CalculateAverageWithinRange(lowerBound int32, upperBound int32, numbers []int32) (float64, error) {
	var sum int32 = 0
	var count int32 = 0
	for _, num := range numbers {
...
func ApplyCaesarCipher(text string, direction string, shift int32) (string, error) {
...
func CountPrimeNumberDigits(numbers []uint32) (uint32, error) {
	var digits uint32 = 0

	for _, num := range numbers {
		if isPrime(int32(num)) {
			digits += CountDigits(int32(num))
		}
	}
	return digits, nil
}
...
func GenerateRandomPasswords(characters []string) ([]string, error) {
	...
	for _, character := range characters {
		if regexp.MustCompile(`[a-z]`).MatchString(character) {
			hasLower = true
		}
		if regexp.MustCompile(`[A-Z]`).MatchString(character) {
			hasUpper = true
		}
		if regexp.MustCompile(`\d`).MatchString(character) {
			hasDigit = true
		}
		if regexp.MustCompile(`[\W_]`).MatchString(character) {
			hasSymbol = true
		}
	}

	if !(hasLower && hasUpper && hasDigit && hasSymbol) {
		return nil, errors.New("input array must contain at least one lowercase, one uppercase, one digit, and one symbol")
	}
