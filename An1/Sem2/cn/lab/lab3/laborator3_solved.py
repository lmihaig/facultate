...
"""
    Obs1. Functii folosite in rezolvare:
    
        - np.triu(matrix) -> Extrage partea superioara a matricei.
            See the docs -> https://numpy.org/doc/stable/reference/generated/numpy.triu.html
            * See also, np.tril(matrix)
        
        - np.all(array) -> Verifica daca toate elementele array-ului au valoarea 'True'.
            See the docs -> https://numpy.org/doc/stable/reference/generated/numpy.all.html?highlight=all#numpy.all
            
        - np.diagonal(matrix) / matrix.diagonal() -> Extrage diagonala specificata.
            See the docs -> https://numpy.org/doc/stable/reference/generated/numpy.diagonal.html?highlight=diagonal#numpy.diagonal
            
        - np.prod(array) -> Returneaza produsul elementelor din array.
            See the docs -> https://numpy.org/doc/stable/reference/generated/numpy.prod.html?highlight=prod#numpy.prod
            
        - np.abs(array) -> Returneaza array cu toate elementele in valoare absoluta.
            See the docs -> https://numpy.org/doc/stable/reference/generated/numpy.absolute.html?highlight=abs#numpy.absolute
        
        - np.full() -> Creaza o matrice cu dimensiunea specificata avand toate elementele egale cu valoarea de umplere specificata.
            See the docs -> https://numpy.org/doc/stable/reference/generated/numpy.full.html?highlight=full#numpy.full
            
        - np.concatenate() -> Concateneaza elementele date pe directia axei specificate.
            See the docs -> https://numpy.org/doc/stable/reference/generated/numpy.concatenate.html?highlight=concatenate#numpy.concatenate
            * See also, np.hstack(), np.vstack(), np.column_stack(), np.row_stack()
            
        - np.any(array) -> Verifica daca macar un element al array-ului este 'True'
            See the docs -> https://numpy.org/doc/stable/reference/generated/numpy.any.html?highlight=any#numpy.any
            
        - np.argmin(array) -> Returneaza indicele elementului cu valoarea cea mai mica din array.
            See the docs -> https://numpy.org/doc/stable/reference/generated/numpy.argmin.html?highlight=argmin#numpy.argmin
            * See also np.argmax(array)
"""

"""
    Obs2. Instructiuni python nou introduse.
    
        - assert -> https://www.w3schools.com/python/ref_keyword_assert.asp

        - raise -> https://www.w3schools.com/python/ref_keyword_raise.asp
            * Table of Built-in Exceptions -> https://www.w3schools.com/python/python_ref_exceptions.asp 
"""
...
def meg_fp(a, b):
    ...
    for i in range(n):  # 0, 1, 2, ... , n-1
        ...
        if not E[i:, i].any():  # Verifica daca sunt zerouri in vectorul de sub pivot (inclusiv)
            raise AssertionError('Sistem incompatibil sau sistem compatibil determinat.')
        else:
